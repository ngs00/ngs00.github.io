...
$$
P(e_m|c_i) = \frac{count(e_m, c_i) + 1}{|c_i| + |u|}
$$

위의 식에서 $$count(e_m, c_i)$$는 training set에서 $$e_m$$이 $$c_i$$일 때 나타나는 횟수이고, &#124;$$c_i$$&#124;와 &#124;$$u$$&#124;는 각각 class $$c_i$$에 있는 모든 element의 수와 모든 클래스에서 유일한 element의 수이다. 예를 들어, Laplace smoothing을 적용한 $$P(int$$&#124;$$Java)$$는 다음과 같이 계산된다.
...
앞의 표 1에서 주어진 training set에는 C/C++, Java라는 두 개의 class가 명시되어 있다. conditional probability를 계산하기 전에 입력 $$s$$에 대해 적절한 전처리를 하여 입력 $$s$$를 [it, has, struct, int, float]이라는 5차원 벡터로 변형하였다고 가정한다. 이러한 전처리 또한 매우 활발히 연구되고 있는 분야이며, 적절한 전처리를 머신러닝 모델의 성능을 크게 향상시킬 수 있다. 전처리된 데이터에 대해 식 (11)을 이용하여 $$P(c_i|\mathcal{D})$$를 다음과 같이 계산한다. 이 예제에서 $$c_1$$은 C/C++, $$c_2$$는 Java를 의미한다.

$$
\begin{aligned}
\log P(c_1|s) \approx & \enspace \log P(it|c_1) + \log P(has|c_1) + \log P(struct|c_1) \\
...
$$
\begin{aligned}
\log P(c_2|s) \approx & \enspace \log P(it|c_2) + \log P(has|c_2) + \log P(struct|c_2) \\
