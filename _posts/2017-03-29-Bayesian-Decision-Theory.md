...
<br />
우리가 어떠한 확률론적 결정을 내리기 위해 필요한 것 중 하나는 *priori probability*라고 불리는 확률이다. 여기에서 말하는 priori는 "선험적인"이라는 뜻을 갖는 단어로써, "이 세상에는 고양이보다 개가 많다" 또는 "이 세상에는 개보다 고양이가 많다"는 것과 같은 선험적인 지식을 말한다. 앞의 예에서 $$P(\omega_1)$$는 주어진 입력이 고양이일 확률을 나타내며, $$P(\omega_2)$$는 개일 확률을 나타낸다. 즉, priori probability는 우리가 입력을 관측하기 전에 알고 있는 지식을 의미한다.
<br />
...
두 개의 class만 존재하는 패턴인식 문제에서 Bayesian decision theory의 기본 개념은 $$P(\omega_1|x)$$가 $$P(\omega_2|x)$$보다 크다면 $$\omega_1$$을 선택하고, 그렇지 않으면 $$\omega_2$$를 선택하는 것이다. 이러한 방법을 따를 경우, 우리가 결정을 내릴 때마다 발생하는 *probability error* $$P(error|x)$$는 다음과 같이 정의된다.

$$
P(error|x) =
\begin{cases}
P(\omega_2|x),  & \text{if $P(\omega_1|x)$ > $P(\omega_2|x)$} \\
P(\omega_1|x), & \text{otherwise}
\end{cases}
$$

한 가지 생각해볼 점은 이러한 결정 방법이 "모든 데이터에 대한 probability error를 최소화하는 것인가?"이다. 모든 probability error를 수학적으로 표현하면 아래의 식 (2)와 같다.
...
<br />
위에서 설명한 바와 같이 $$P(w|x)$$를 이용하여 결정을 내리면, 우리는 probability error를 최소화할 수 있다. 따라서, 이러한 최적의 결정을 내리기 위해서는 입력이 주어질 때마다 정확한 $$P(w|x)$$를 계산할 수 있어야 한다. 실제 문제에서는 $$P(w|x)$$를 직접 계산하는 것은 매우 어렵거나 불가능하기 때문에 우리는 식 (3)과 같이 정의되는 *Bayes' theorem*을 이용하여 간접적으로 $$P(w|x)$$를 계산한다.
...
P(error|x) =
\begin{cases}
P(\omega_2|x),  & \text{if $p(x|\omega_1)P(\omega_1)$ > $p(x|\omega_2)P(\omega_2)$} \\
P(\omega_1|x), & \text{otherwise}
\end{cases}
$$
...
<br />
어떠한 state $$\omega_j$$에서 특정한 action $$\alpha_i$$를 할 때의 *loss*은 $$\lambda(\alpha_i|\omega_j)$$로 정의되며, loss의 expected value를 *risk* (또는 *expected loss*)라고 한다. 주어진 입력 $$x$$에 대한 risk를 의미하는 *conditional risk*는 아래의 식 (5)와 같이 정의된다.

$$
R(\alpha_i|\boldsymbol{x}) = \sum_{j=1}^{C} \lambda(\alpha_i|\omega_j)P(\omega_j|x)
$$
...
어떠한 classifier를 설계할 때, 가장 직관적이고 단순한 방법은 *discriminant function* $$g(\boldsymbol{x}, \omega_i)$$를 이용하는 것이다. 만약, 주어진 입력 $$\boldsymbol{x}$$에 대해서 다음의 식 (7)이 성립한다면, classifier는 $$\boldsymbol{x}$$를 $$\omega_i$$로 할당할 것이다.

$$
g(\boldsymbol{x}, \omega_i) > g(\boldsymbol{x}, \omega_j) \qquad \text{for all} i /neq j
$$

Discriminant function $$g(\boldsymbol{x}, \omega_i)$$는 classifier의 설계 목적에 따라 여러 형태로 정의될 수 있다. 앞에서 정의한 conditional risk를 이용하여 정의한다면, $$g(\boldsymbol{x}, \omega_i) = -R(\alpha_i|\boldsymbol{x})$$의 형태로 discriminant function을 표현할 수 있다. 만약 어떠한 함수 $$f$$가 단조증가함수 (monotonically increasing function)라면, discriminant function을 $$f(g(\omega_i|\boldsymbol{x}))$$로 정의하여도 decision rule은 변하지 않는다. 이러한 변환은 classifer의 결과에 영향을 주지 않으면서, 계산을 쉽게 만들기 위해 많이 사용된다.
<br />
...
위의 그림에서 각 decision region의 경계를 *decision boundary*라고 하며, 이러한 경계는 discriminant function $$g(\boldsymbol{x}, \omega_i)$$에 의해 정의된다.
<br />
